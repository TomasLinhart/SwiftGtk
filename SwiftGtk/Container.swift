...
public class Container: Widget {
    ...
    public func add(widget: Widget) {
        ...
        gtk_container_add(UnsafeMutablePointer<GtkContainer>(widgetPointer), widget.widgetPointer)
    }
    
    public func remove(widget: Widget) {
        if let index = widgets.indexOf({$0 === widget}) {
            gtk_container_remove(UnsafeMutablePointer<GtkContainer>(widgetPointer), widget.widgetPointer)
